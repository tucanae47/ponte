...
CoAP.prototype._handlePUT = function(topic, payload, res) {
  ...
  var packet = { topic: topic, payload: unpacked_payload, retain: true };

  that._persistence.storeRetained(packet, function() {
    that._broker.publish(topic, String(unpacked_payload), {}, function() {
     
      res.setOption('Location-Path', '/r/' + topic);
      res.statusCode = '2.04';
      res.end();

      that._ponte.emit('updated', topic, unpacked_payload);
    });
  });
};
